...
def round_obs(x1,y1):
	...
	for i in range(0,360,10):
		obs[0].append(x1+10*math.cos(i/180*math.pi))
		obs[1].append(y1+10*math.sin(i/180*math.pi))

def if_safe(id,x1,y1):
	global boat
	if math.sqrt((boat[0]-x1)**2+(boat[1]-y1)**2)<20:
		return False
	else:
		return True
...
def planning(x_c,x_s,N_pre):
	...
	for i in range(N):
		...
		obj = obj + 3*state_error + control_error + 10 * horizon_error + 10000*dynamic_error
		#g.append(X[:, i + 1] - x_next_)

	#### obsatcle constraints
	for i in range(N + 1):
		for j in range(len(obs[0])):
			obj = obj + 20000/ca.sqrt((X[0, i] - obs[0][j]) ** 2 + (X[1, i] - obs[1][j]) ** 2)
...
if __name__ == '__main__':
	...
	target_num = 0
	stop_flag = [False,False,False,False,False,False,False,False]
	had_arrive_the_dis = False
	if_arrived_current = False
	decide_angle = 0
	points_angle = []
	x1,y1 = gps_to_mkt1(113.69964901,22.01968619)
	x2,y2 = gps_to_mkt1(113.69898723,22.02014708)
	x3,y3 = gps_to_mkt1(113.69795922,22.02037745)
