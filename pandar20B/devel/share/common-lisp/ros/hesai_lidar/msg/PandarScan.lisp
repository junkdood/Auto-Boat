...
(cl:defclass <PandarScan> (roslisp-msg-protocol:ros-message)
  ((header
   ...
   (packets
    :reader packets
    :initarg :packets
    :type (cl:vector hesai_lidar-msg:PandarPacket)
   :initform (cl:make-array 0 :element-type 'hesai_lidar-msg:PandarPacket :initial-element (cl:make-instance 'hesai_lidar-msg:PandarPacket))))
)
...
(cl:defmethod cl:initialize-instance :after ((m <PandarScan>) cl:&rest args)
  ...
  (cl:unless (cl:typep m 'PandarScan)
    (roslisp-msg-protocol:msg-deprecation-warning "using old message class name hesai_lidar-msg:<PandarScan> is deprecated: use hesai_lidar-msg:PandarScan instead.")))

(cl:ensure-generic-function 'header-val :lambda-list '(m))
(cl:defmethod header-val ((m <PandarScan>))
  (roslisp-msg-protocol:msg-deprecation-warning "Using old-style slot reader hesai_lidar-msg:header-val is deprecated.  Use hesai_lidar-msg:header instead.")
  (header m))

(cl:ensure-generic-function 'packets-val :lambda-list '(m))
(cl:defmethod packets-val ((m <PandarScan>))
  (roslisp-msg-protocol:msg-deprecation-warning "Using old-style slot reader hesai_lidar-msg:packets-val is deprecated.  Use hesai_lidar-msg:packets instead.")
  (packets m))
...
(cl:defmethod roslisp-msg-protocol:deserialize ((msg <PandarScan>) istream)
  ...
  (cl:let ((vals (cl:slot-value msg 'packets)))
    ...
    (cl:setf (cl:aref vals i) (cl:make-instance 'hesai_lidar-msg:PandarPacket))
...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<PandarScan>)))
  ...
  (cl:format cl:nil "Header header~%PandarPacket[] packets~%~%================================================================================~%MSG: std_msgs/Header~%# Standard metadata for higher-level stamped data types.~%# This is generally used to communicate timestamped data ~%# in a particular coordinate frame.~%# ~%# sequence ID: consecutively increasing ID ~%uint32 seq~%#Two-integer timestamp that is expressed as:~%# * stamp.sec: seconds (stamp_secs) since epoch (in Python the variable is called 'secs')~%# * stamp.nsec: nanoseconds since stamp_secs (in Python the variable is called 'nsecs')~%# time-handling sugar is provided by the client library~%time stamp~%#Frame this data is associated with~%string frame_id~%~%================================================================================~%MSG: hesai_lidar/PandarPacket~%# field		size(byte)~%# SOB 		2~%# angle		2~%# measure	5~%# block		SOB + angle + measure * 40~%# timestamp	4~%# factory	2~%# reserve	8~%# rpm		2~%# tail		timestamp + factory + reserve + rpm~%# packet	block * 6 + tail~%~%time stamp~%uint8[1500] data~%uint32 size~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'PandarScan)))
  "Returns full string definition for message of type 'PandarScan"
  (cl:format cl:nil "Header header~%PandarPacket[] packets~%~%================================================================================~%MSG: std_msgs/Header~%# Standard metadata for higher-level stamped data types.~%# This is generally used to communicate timestamped data ~%# in a particular coordinate frame.~%# ~%# sequence ID: consecutively increasing ID ~%uint32 seq~%#Two-integer timestamp that is expressed as:~%# * stamp.sec: seconds (stamp_secs) since epoch (in Python the variable is called 'secs')~%# * stamp.nsec: nanoseconds since stamp_secs (in Python the variable is called 'nsecs')~%# time-handling sugar is provided by the client library~%time stamp~%#Frame this data is associated with~%string frame_id~%~%================================================================================~%MSG: hesai_lidar/PandarPacket~%# field		size(byte)~%# SOB 		2~%# angle		2~%# measure	5~%# block		SOB + angle + measure * 40~%# timestamp	4~%# factory	2~%# reserve	8~%# rpm		2~%# tail		timestamp + factory + reserve + rpm~%# packet	block * 6 + tail~%~%time stamp~%uint8[1500] data~%uint32 size~%~%"))
