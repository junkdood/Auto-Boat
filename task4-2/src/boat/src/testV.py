...
def if_dynamic(id,x1,y1):
	if sim[id][0] != x1 or sim[id][1] != y1:
		return True 
	else:
		return False
...
def round_obs(x1,y1):
	...
	for i in range(0,360,20):
		obs[0].append(x1+12*math.cos(i/180*math.pi))
		obs[1].append(y1+12*math.sin(i/180*math.pi))

def if_safe(id,x1,y1):
	global boat
	obs_x_step = x1 - sim[id][0] 
	obs_y_step = y1 - sim[id][1] 
	boat_x_step = boat[0] - last_boat0
	boat_y_step = boat[1] - last_boat1
	obsx = x1
	obsy = y1
	boatx = boat[0]
	boaty = boat[1]
	...
	for i in range(1500):
		...
		boatx += boat_x_step
		boaty += boat_y_step
		# boat_pointx.append(boatx)
		# boat_pointy.append(boaty)
		# obs_pointx.append(obsx)
		# obs_pointy.append(obsy)
		if math.sqrt((obsx - boatx)**2+(obsy - boaty)**2)<=20:
			return False
	return True
...
class mpc_stanly_com(object):
	...
	def listener(self):
		...
		rospy.Subscriber("simulated",NavSatFix,self.get_simulated)
	...
	def get_simulated(self,msg):
		global obs,stop_flag,boat
		id = int(msg.position_covariance[1])
		if id == 64:
			id = 6
		if id == 65:
			id = 7
		x1, y1 = gps_to_mkt1(msg.longitude,msg.latitude)
		if sim[id][0] == 0 and sim[id][1] == 0:
			sim[id][0] = x1
			sim[id][1] = y1
		else:
			if if_dynamic(id,x1,y1):
				dist = math.sqrt((boat[0] - x1)**2 +(boat[1] - y1)**2)
				print(dist)
				if dist < 100:
					if if_safe(id,x1,y1):
						print("strat recv sim***************************************************************************\n")
						stop_flag[id] = False
					else:
						stop_flag[id] = True
				sim[id][0] = x1
				...
				flag = 0
				for i in range(len(obs[0])):
					dist = math.sqrt((x1-obs[0][i])**2+(y1-obs[1][i])**2)
					if dist < 1:
						flag = 1
				if flag == 0:
					obs[0].append(x1)
...
def plot_ponit():
	...
	while True:
		plt.clf()
		plt.axis('equal')
		plt.xlim(boat[0] - 60, boat[0] + 60)
		plt.ylim(boat[1] - 60, boat[1] + 60)
		plt.xlabel(boat[0])
		plt.ylabel(boat[1])
		plt.title(goal_num)
		...
		for i in range(len(sim)):
			plt.plot(sim[i][0],sim[i][1],'yo')
		plt.plot(obs[0],obs[1], "ro")

		# plt.plot(boat_pointx,boat_pointy)
		# plt.plot(obs_pointx,obs_pointy)
...
if __name__ == '__main__':
	...
	target_num = 0
	stop_flag = [False,False,False,False,False,False]
	had_arrive_the_dis = False
	if_arrived_current = False
	decide_angle = 0
	points_angle = []
	x1,y1 = gps_to_mkt1(113.69964901,22.01968619)
	x3,y3 = gps_to_mkt1(113.69895163,22.02005917)
	x5,y5 = gps_to_mkt1(113.69795922,22.02037745)

	theta1 = math.atan2(y3 - y1,x3 - x1)
	theta2 = theta1+math.pi/4
	theta3 = theta1-math.pi/4

	x2 = x1 + math.sqrt((x3-x1)**2+(y3-y1)**2)*math.cos(theta2)*2/(math.sqrt(2))
	y2 = y1 + math.sqrt((x3-x1)**2+(y3-y1)**2)*math.sin(theta2)*2/(math.sqrt(2))
	x4 = x1 + math.sqrt((x3-x1)**2+(y3-y1)**2)*math.cos(theta3)*2/(math.sqrt(2))
	y4 = y1 + math.sqrt((x3-x1)**2+(y3-y1)**2)*math.sin(theta3)*2/(math.sqrt(2))
